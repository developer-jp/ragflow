...
class VisionParser(RAGFlowPdfParser):
    def __init__(self, vision_model, *args, **kwargs):
        ...
        self.outlines = []  # VisionParser doesn't extract outlines
        ...
        try:
            with sys.modules[LOCK_KEY_pdfplumber]:
                ...
                for i, p in enumerate(self.pdf.pages[page_from:page_to]):
                    # Method 1: Try to use to_image and get the PIL Image
                    try:
                        page_img = p.to_image(resolution=72 * zoomin)
                        # Save to bytes and reload as clean PIL Image
                        ...
                        logging.warning(f"Failed to convert page {i} to image: {e}")
                        # Create a blank image as fallback
                        blank_img = Image.new("RGB", (850, 1100), color="white")
                        self.page_images.append(blank_img)

                self.total_page = len(self.pdf.pages)
        except Exception as e:
            self.page_images = None
            self.total_page = 0
            logging.exception(f"VisionParser __images__ error: {e}")
    ...
    def __call__(self, filename, from_page=0, to_page=100000, **kwargs):
        ...
        for idx, img_binary in enumerate(self.page_images or []):
            pdf_page_num = from_page + idx  # Adjust for actual page number
            if pdf_page_num >= end_page:
                continue

            # Use detailed prompt for better structure
            detailed_prompt = """Please extract the layout information from the PDF page image and output only the text content in Markdown format.
...
2. Categories:
   ...
   - All other categories (Text, Title, Caption, etc.): output as Markdown with proper headings.

3. Constraints:
   - The output text must be the original text from the image, with no translation.
   - All layout elements must be sorted according to human reading order.
   - Tables should be formatted as complete HTML tables.
   - Use proper Markdown headers (# ## ###) for titles and section headings.
   - Structure content logically with paragraphs and line breaks.

4. Final Output: A single Markdown document containing the extracted content with tables in HTML format.

DO NOT add page markers or dividers. Only output the clean, structured content."""
            ...
            if text:
                ...
                # Return format compatible with manual mode: (text, layout_level, position_info)
                # Position format: [(page_num, left, right, top, bottom)]
                position_info = [(pdf_page_num + 1, 0, width / zoomin, 0, height / zoomin)]
                all_docs.append((text, 0, position_info))  # level=0 for full page content
        return all_docs, []
