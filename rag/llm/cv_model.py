...
class GeminiCV(Base):
    ...
    def describe_with_prompt(self, image, prompt=None):
        ...
        b64_data_url = self.image2base64(image)
        vision_prompt = prompt if prompt else vision_llm_describe_prompt()

        # Extract base64 data from data URL if needed
        if b64_data_url.startswith("data:") and ";base64," in b64_data_url:
            # Split off the data URL prefix to get just the base64 part
            b64_str = b64_data_url.split(";base64,")[1]
        else:
            b64_str = b64_data_url

        img = open(BytesIO(base64.b64decode(b64_str)))
